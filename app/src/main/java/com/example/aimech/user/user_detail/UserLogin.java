            ...
            firebaseDatabase.getReference().child("Users").child(userID).child("user_type")
                    .addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            int usertype = snapshot.getValue(Integer.class); //get user type and save it on the variable

                            progressDialog.dismiss(); //dismiss the progress dialog

                            if (usertype == 0) {
                                //pass the intent if user type is correct
                                startActivity(new Intent(getApplicationContext(), UserDashboard.class));
                                finish();
                            } else {
                                //show toast message if user type incorrect
                                Toast.makeText(UserLogin.this, "Your account type is incorrect", Toast.LENGTH_SHORT).show();
                            }
                        }
                    ...
                    auth.signInWithEmailAndPassword(userEnteredEmail, userEnteredPassword).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            ...
                            if (task.isSuccessful()) {
                                ...
                                if (user.isEmailVerified()) { //check email verified or not

                                    progressDialog.show(); //show progress dialog while process
                                    String userID = task.getResult().getUser().getUid(); //get user id
                                            ...
                                            .addListenerForSingleValueEvent(new ValueEventListener() {
                                                ...
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                    ...
                                                    if (usertype == 0) {
                                                        ...
                                                        Toast.makeText(UserLogin.this, "Login failed, your account type is incorrect", Toast.LENGTH_SHORT).show();
                                                    }
                                                }
                            ...
                            //login failed
                                ...
                                progressDialog.dismiss();
                                Toast.makeText(UserLogin.this, "Logging Failed " + task.getException().getMessage(), Toast.LENGTH_LONG).show();
                            }
                        }
                    });
                }
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        auth.sendPasswordResetEmail(mail).addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                Toast.makeText(UserLogin.this, "Reset Link sent to your mail", Toast.LENGTH_LONG).show();
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(UserLogin.this, "Error ! Rest Link Not Sent " + e.getMessage(), Toast.LENGTH_LONG).show();
                            }
                        });
                    }
                });
